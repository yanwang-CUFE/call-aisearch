...
class BertAISearchModel2(nn.Module):

    def __init__(self):
        super(BertAISearchModel2, self).__init__()
...
class SimilarQueryModel():
    """
    call model return json
    """
    def __init__(self, model, tokenizer, all_matrix, device, query_dict):
        """
    ...
    def get_top_1(self, simi_matrix):
        ...
        for i in range(int(col)):
            ...
            if (simi_matrix[0][i] > max_cos):
                ...
                print(i)
                print(max_cos)
                print(self.query_dict[str(max_index)]["query"])
        print("loop end!")
        print(max_index)
        # dict_return = self.query_dict[str(max_index)]
        print(dict_return)
